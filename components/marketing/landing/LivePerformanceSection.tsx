...
export function LivePerformanceSection() {
  ...
  const [ctaHref, setCtaHref] = useState<string>("/dashboard");
  ...
  useEffect(() => {
    const loadAccess = async () => {
      try {
        ...
        if (!a.is_logged_in) setCtaHref("/signup");
        else if (!a.is_pro && !a.is_trial) setCtaHref("/pricing");
        else setCtaHref("/dashboard");
      } catch {
