/**
 * DAYTRADER V4 Backtest Engine (Order Blocks + Liquidity Sweep)
 * 
 * Full isolated backtest for V4 engine using:
 * - BOS/CHOCH detection
 ...
 * 
 * Target tickers: META, COIN, IWM, TSLA, AMD, RIVN
 */

import { OHLCBar } from './signal_types.ts';
import { evaluateDaytraderEntryV4 } from './daytrader_entry_v4.ts';
...
export interface DaytraderV4BacktestParams {
  symbol: string;
  bars: OHLCBar[];   // 5m intraday bars
}
...
/**
 * V4-specific levels: tighter stops based on order block wicks
 * SL = OB wick extreme
 * TP1 = 1.5R
 * TP2 = 3R
 */
function calculateV4Levels(
  ...
  // V4 uses tighter stops: 0.4x ATR (vs 0.5x for V3)
...
/**
 * Check if position hit +1R for breakeven move
 */
...
export async function runDaytraderBacktestV4(
  params: DaytraderV4BacktestParams
  ...
  // Start after sufficient lookback (250 bars = ~20 hours of 5m data)
  for (let i = 250; i < bars.length; i++) {
    ...
    if (position) {
      ...
      // Conservative ordering: SL before TP if both hit
    ...
    if (!position && !equityZero) {
      ...
      // Lookback window for V4 entry evaluation
      const lookbackBars5m = bars.slice(Math.max(0, i - 250), i + 1);
      if (lookbackBars5m.length < 50) continue;

      // *** V4 ENTRY LOGIC ***
      const entrySignal = evaluateDaytraderEntryV4(lookbackBars5m);
      ...
      // Calculate ATR and position levels
  ...
  // Close any remaining position at last bar (mark to market)
  if (position) {
