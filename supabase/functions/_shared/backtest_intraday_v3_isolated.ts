/**
 * Isolated DAYTRADER v3 Backtest Engine
 * 
 * ONLY uses evaluateDaytraderEntryV3 - no v3.5 logic
 * Duplicate of backtest_intraday_experimental.ts but with v3 entry logic hardcoded
 */

import { OHLCBar } from './signal_types.ts';
import { evaluateDaytraderEntryV3 } from './daytrader_entry_v3.ts';
...
export interface IntradayBacktestV3Result {
...
export async function runIntradayDaytraderBacktestV3(params: {
  symbol: string;
  bars: OHLCBar[];
}): Promise<IntradayBacktestV3Result> {
  ...
  for (let i = 250; i < bars.length; i++) {
    ...
    if (!position && !equityZero) {
      ...
      // V3 ENTRY LOGIC ONLY
      const entrySignal = evaluateDaytraderEntryV3(lookbackBars5m);
