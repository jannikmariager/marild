...
export interface BacktestStatsV4 {
  ...
  // V4.7-specific HTF alignment diagnostics (absent in V4.6 baseline)
  htf_filtered_trades?: number;
  htf_filtered_longs?: number;
  htf_filtered_shorts?: number;
  htf_bias_distribution?: { bullish: number; bearish: number; neutral: number };
}
...
export async function runBacktestV4(
  ...
  const stats: BacktestStatsV4 & { equity_curve: Array<{ t: number; balance: number }> } = {
    ...
    htf_filtered_trades: exec.htfFilteredTrades,
    htf_filtered_longs: exec.htfFilteredLongs,
    htf_filtered_shorts: exec.htfFilteredShorts,
    htf_bias_distribution: exec.htfBiasDistribution,
  };
