// supabase/functions/_shared/backtest/v6_0/trend_daytrader_v60.ts
//
// ENGINE A: Trend continuation micro-engine for v6.0 DAYTRADER.
//
// This reuses the v4.8 SMC engine signal feed (evaluateSignalV48)
// and applies trend/volatility/SMC gating tuned for clear intraday trends.
...
import type { DayEngineContextV60 } from "./daytrader_router_v60.ts";

export interface TradeV60 {
  entryTime: string;
  exitTime: string;
  entryPrice: number;
  exitPrice: number;
...
export interface ExecutionResultV60 {
  trades: TradeV60[];
  ...
  lastMetadata?: {
    engine_version: string; // e.g. "v6.0/trend"
    behavior: string;
    engine_id: string;
    regime?: string;
...
export async function runTrendDaytraderV60(
  engineType: EngineType,
  symbol: string,
  bars: OHLCBar[],
  ...
  const engine_version = "v6.0/trend";

  const trades: TradeV60[] = [];
  ...
  let lastMetadata: ExecutionResultV60["lastMetadata"];
  ...
  for (let i = 0; i < bars.length - 1; i++) {
    ...
    if (!openPos && engineType === "DAYTRADER") {
      ...
      // Trend engine gating: require clear trend and non-extreme volatility
      ...
      if (trendStrength < 30) {
        filteredSignals++;
        filterReasons["trend_weak_v60_trend"] = (filterReasons["trend_weak_v60_trend"] ?? 0) + 1;
        continue;
      }
      if (volState === "extreme") {
        filteredSignals++;
        filterReasons["vol_extreme_v60_trend"] =
          (filterReasons["vol_extreme_v60_trend"] ?? 0) + 1;
        continue;
      }
      ...
      if (regime === "RANGE") {
        filteredSignals++;
        filterReasons["range_block_v60_trend"] =
          (filterReasons["range_block_v60_trend"] ?? 0) + 1;
        continue;
      }
      ...
      if (size <= 0) {
        filteredSignals++;
        filterReasons["size_zero_v60_trend"] = (filterReasons["size_zero_v60_trend"] ?? 0) + 1;
        continue;
      }
