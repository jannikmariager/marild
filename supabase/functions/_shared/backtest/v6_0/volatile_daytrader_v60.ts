// supabase/functions/_shared/backtest/v6_0/volatile_daytrader_v60.ts
//
// ENGINE C: High-volatility / breakout micro-engine for v6.0 DAYTRADER.
...
import type { ExecutionResultV60, TradeV60 } from "./trend_daytrader_v60.ts";
...
export async function runVolatileDaytraderV60(
  engineType: EngineType,
  symbol: string,
  bars: OHLCBar[],
  ...
  const engine_version = "v6.0/volatile";
  ...
  for (let i = 0; i < bars.length - 1; i++) {
    ...
    if (!openPos && engineType === "DAYTRADER") {
      ...
      if (volState === "extreme" && trendStrength < -40) {
        filteredSignals++;
        filterReasons["meltdown_block_v60_volatile"] =
          (filterReasons["meltdown_block_v60_volatile"] ?? 0) + 1;
        continue;
      }
      ...
      if (!compressedRecently || !expandingNow) {
        filteredSignals++;
        filterReasons["no_compress_expand_v60_volatile"] =
          (filterReasons["no_compress_expand_v60_volatile"] ?? 0) + 1;
        continue;
      }
      ...
      if (!volume.spike && !volume.climax) {
        filteredSignals++;
        filterReasons["no_volume_spike_v60_volatile"] =
          (filterReasons["no_volume_spike_v60_volatile"] ?? 0) + 1;
        continue;
      }
      ...
      if (size <= 0) {
        filteredSignals++;
        filterReasons["size_zero_v60_volatile"] =
          (filterReasons["size_zero_v60_volatile"] ?? 0) + 1;
        continue;
      }
