// supabase/functions/_shared/backtest/v6_0/range_daytrader_v60.ts
//
// ENGINE B: Range / reversal micro-engine for v6.0 DAYTRADER.
// Focuses on liquidity sweeps + CHoCH and modest RR.
...
export async function runRangeDaytraderV60(
  engineType: EngineType,
  symbol: string,
  bars: OHLCBar[],
  ...
  const engine_version = "v6.0/range";
  ...
  for (let i = 0; i < bars.length - 1; i++) {
    ...
    if (!openPos && engineType === "DAYTRADER") {
      ...
      if (trendStrength > 40 && volState !== "low") {
        filteredSignals++;
        filterReasons["too_trendy_v60_range"] =
          (filterReasons["too_trendy_v60_range"] ?? 0) + 1;
        continue;
      }
      ...
      if (!hasSweep && !hasChoch) {
        filteredSignals++;
        filterReasons["no_sweep_no_choch_v60_range"] =
          (filterReasons["no_sweep_no_choch_v60_range"] ?? 0) + 1;
        continue;
      }
      ...
      if (size <= 0) {
        filteredSignals++;
        filterReasons["size_zero_v60_range"] = (filterReasons["size_zero_v60_range"] ?? 0) + 1;
        continue;
      }
