/**
 * BACKTEST ENGINE V4.8 - CORE
 *
 * Modular SMC engine orchestration for backtesting.
 * Uses the new engine/ modules (SMC, trend, volume, liquidity, volatility, risk, exits, confluence).
 * Isolated from v4.6 and v4.7 to preserve existing behavior.
 */
...
import { runExecutionModelV48, TradeV4, ExecutionResult } from "./execution_model_v48.ts";
...
export async function runBacktestV4(
  ...
  if (insufficient) {
    throw new Error(
      `[BacktestV4.8] INSUFFICIENT_DATA for ${symbol}/${timeframe}: ${bars.length} sanitized bars`,
    );
  }

  // 2. Run v4.8 execution model
  const exec: ExecutionResult = await runExecutionModelV48(
    engine_type,
    symbol,
    bars,
    startingEquity,
